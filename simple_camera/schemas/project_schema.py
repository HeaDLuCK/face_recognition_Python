...
class CameraAssignment(GenBaseModel):
    etsAuth: str
    enabled: bool = True
...
class CameraConfig(GenBaseModel):
    ...
    def normalize_assignments(self):
        if not self.assignments:
            if not self.etsAuth:
                raise ValueError("Camera requires etsAuth or assignments")
            self.assignments = [
                CameraAssignment(
                    etsAuth=self.etsAuth,
        ...
        assignments_by_etsAuth = {
            assignment.etsAuth: assignment
            for assignment in self.assignments
        }
        self.assignments = list(assignments_by_etsAuth.values())
        ...
        self.etsAuth = primary_assignment.etsAuth
    ...
    @property
    def listOfEtsAuths(self) -> list[str]:
        return [assignment.etsAuth for assignment in self.assignments]
...
class EmployeeConfig(GenBaseModel):
    etsAuth: str
    employeeId: str
...
class EventPayload(GenBaseModel):
    etsAuth: str
    cameraId: str
    eventType: str
