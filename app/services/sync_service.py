...
<<<<<<< HEAD
    async def _decode_face_image(self, ref) -> bytes | None:
        ...
        if not inline:
            if ref.imageUrl or ref.url:
                raise ValueError(
                    "Face image URLs are not supported; send the image in base64 or content"
                )
            return None

        encoded = inline.strip()
        if encoded.lower().startswith("data:"):
            if "," not in encoded:
                raise ValueError("Employee face image has an invalid data URL")
            header, encoded = encoded.split(",", 1)
            if ";base64" not in header.lower():
                raise ValueError("Employee face image data URL is not Base64 encoded")

        encoded = "".join(encoded.split())
        if not encoded:
            raise ValueError("Employee face image Base64 data is empty")
